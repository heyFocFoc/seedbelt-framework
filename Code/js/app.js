...
!function (a, b) {
    ...
    function a(a) {
        ...
        var b = m.call(a);return "[object String]" === b ? l(a) : c(a) && /^\[object (Array|HTMLCollection|NodeList|Object)\]$/.test(b) && a.hasOwnProperty("length") ? a : d(a) ? [a] : [];
    }function f(a) {
        var c,
            d,
            e = { top: 0, left: 0 },
        ...
        var b = i(a);if (null !== b) {
            if (b.disabled || b.getAttribute("disabled") || b.classList.contains("disabled")) return;if (q.registerEvent(a), "touchstart" === a.type && o.delay) {
                var c = !1,
...
// carousel arrows
var navArrows = ["\n    <i class=\"material-icons\">arrow_back</>", "<i class=\"material-icons\">arrow_forward</>"];

// btn waves effect
(function () {
    Waves.attach('[data-wave]', ['waves-button']);
    Waves.init();
})();

// main bg video size
(function () {
    var video = $('.js-bg-video');
    if (video.length) {
        var resizeVideo = function resizeVideo() {
            var wndWidth = wnd.width(),
                wndHeight = wnd.height(),
                newWidth = wndHeight * iframeWidth / iframeHeight,
                newHeight = wndWidth * iframeHeight / iframeWidth;

            if (wndWidth <= newWidth) {
                iframe.css({ 'width': newWidth, 'height': wndHeight });
            } else {
                iframe.css({ 'width': wndWidth, 'height': newHeight });
            }
        };

        var iframe = $('.js-bg-iframe'),
...
(function () {
    ...
    if (anim.length) {
        ...
        anim.each(function () {
            ...
            if (!_this.parents('.hidden').length) {
                var changeBackground = function changeBackground() {
                    var posTop = _this.offset().top,
                        posScroll = wnd.scrollTop(),
                        wndHeight = wnd.height(),
                        blockHeight = _this.height(),
                        dataBg = _this.data('bg');

                    if (dataBg) {
                        if (posScroll + wndHeight / 2 >= posTop && posScroll + wndHeight / 2 <= posTop + blockHeight) {
                            if (!_this.hasClass('current')) {
                                anim.removeClass('current');
                                _this.addClass('current');
                                bgItem.removeClass('visible');
                                bgItem.eq(dataBg - 1).addClass('visible');
                            }
                        }
                    }
                };
        ...
        // preload pictures
        var preloadPictures = function preloadPictures(pictureUrls, callback) {
            var i,
                j,
                loaded = 0;
...
(function () {
    ...
    if (lightbox.length) {
        ...
        var img = lightbox.find('.js-lightbox-img'),
            iframe = lightbox.find('.js-lightbox-iframe'),
            body = $('body');

        $('[data-lightbox-img]').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();

            body.addClass('no-scroll');

            var src = $(this).data('lightbox-img');
        ...
        $('[data-lightbox-video]').on('click', function (e) {
            ...
            var src = $(this).data('lightbox-video');
...
(function () {
    var didScroll,
        lastScrollTop = 0;
    delta = 70, header = $('.js-header'), footer = $('.js-footer'), navbarHeight = header.outerHeight();
    ...
    function hasScrolled() {
        ...
        if (st > lastScrollTop && st > navbarHeight) {
            ...
            // end page
            if (st + windowHeight + 100 >= bodyHeight) {
...
(function () {
    $('[data-add]').on('click', function (e) {
        ...
        var _this = $(this),
            block = $(_this.data('add')),
            margin = 20;

        block.show();
        setTimeout(function () {
            $('html, body').animate({
                scrollTop: block.offset().top - margin
            }, 300);

            // animate blocks
            setTimeout(function () {
                var childs = block.find('[data-animate]'),
                    count = 0;
                setInterval(function () {
                    childs.eq(count).addClass('animated');
                    count++;
                }, 100);
            }, 400);
        }, 100);
